...
class capsNetwork3D(nn.Module):
    def __init__(self, agents, frame_history, number_actions, xavier=True):
        super(capsNetwork3D, self).__init__()
        self.agents = agents
        self.frame_history = frame_history
        self.number_actions = number_actions
...
class DigitCapsules(nn.Module):
    def __init__(self):
        ...
        self.W = nn.Parameter(torch.randn(10, 32 * 6 * 6, 16, 8))

    def forward(self, x):
        batch_size = x.size(0)
        u_hat = torch.matmul(x[:, None, :, None, :], self.W[None, :, :, :, :])
        u_hat = u_hat.view(batch_size, 32 * 6 * 6, 10, 16, 1)
        b = torch.zeros(batch_size, 32 * 6 * 6, 10, 1, device=x.device)
        for i in range(self.routing_iterations):
            c = nn.Softmax(dim=2)(b)
            s = (c * u_hat).sum(dim=1, keepdim=True)
            v = self.squash(s)
            if i < self.routing_iterations - 1:
                agreement = torch.matmul(u_hat.transpose(3, 4), v)
...
class capsCommNet(nn.Module):

    def __init__(self, agents, frame_history, number_actions, xavier=True, attention=False):
        super(capsCommNet, self).__init__()
...
class DQN:
    ...
    def __init__(
        ...
        # Create a Q-network, which predicts the q-value for a particular state
        if type == "Network3d":
            self.q_network = capsNetwork3D(
                agents,
                frame_history,
                number_actions).to(
                self.device)
            self.target_network = capsNetwork3D(
                agents, frame_history, number_actions).to(
                self.device)
        elif type == "CommNet":
            self.q_network = capsCommNet(
                agents,
                frame_history,
                number_actions,
                attention=attention).to(
                self.device)
            self.target_network = capsCommNet(
                agents,
                frame_history,
                number_actions,
                attention=attention).to(
